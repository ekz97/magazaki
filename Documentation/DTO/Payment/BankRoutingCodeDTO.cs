...
{
    // BeneficiariesDTO myDeserializedClass = JsonSerializer.Deserialize<BeneficiariesDTO>(myJsonResponse);
    public class BankRoutingCodeDTO
    {
        [JsonConstructor]
        public BankRoutingCodeDTO(
            string routingCodeKey,
            string routingCodeValue
        )
        {
            this.routingCodeKey = routingCodeKey;
            this.routingCodeValue = routingCodeValue;
        }

        [JsonPropertyName("routingCodeKey")]
        public string routingCodeKey { get; }

        [JsonPropertyName("routingCodeValue")]
        public string routingCodeValue { get; }
    }
